...
def mergeSpecesAndFeature():
    ...
    mixed_features.to_csv("/home/chuand/small_orf/data/AllSpeciesMixedFeature.csv")
...
out = open("/home/chuand/small_orf/result/FourToOne", "w")
n_estimators = 170 
max_depth = 19
...
pfiles.append("coding_sORF_seq_uniq_v1.fa")
nfiles.append("noncoding_sORF_seq_uniq_v1.fa")
data = pd.read_csv("/home/chuand/small_orf/data/AllSpeciesMixedFeature.csv")
data['Labels'] = (data['Labels'] == "coding").astype(int)
for i in range(0, 4):
    c = pfiles[i]
    nc = nfiles[i]
    prediction = data[(data["file"] == nc) | (data["file"] == c)]
    Y_ = prediction["Labels"]
    X_prediction = prediction.iloc[:,1:-2]

    tmptrain = data[data["file"] != nc]
    train = tmptrain[tmptrain["file"] != c]
